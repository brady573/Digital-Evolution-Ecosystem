...
`repo-ai` is a vendor-neutral local policy checker. It recognizes a strictly defined policy YAML subset: `schema: repo-ai/v1`, `rules:`, and each rule's `id`, `level`, and `requirement`. Unsupported fields and duplicate rule IDs are errors. Rule IDs are lowercase dotted identifiers. Levels are `guidance` (informational, nonblocking), `check` (diagnostic, nonblocking), and `enforce` (blocking). Findings are sorted by rule ID, path, and message for stable JSON output. Exit code 0 includes guidance and check findings; exit code 1 means an enforce finding; exit code 2 means invalid configuration, policy or lock, or an internal evaluation error.
