 ...
 * WASTE_HALF_SAT is a half-saturation constant in waste-fraction units for
...
class WasteField{
 ...
 declare produced:number;declare bioRemoved:number;declare decayed:number;declare clampAdj:number;declare discarded:number;
 ...
 constructor(totalCap:number,updateStride:number){
  ...
  this.produced=0;this.bioRemoved=0;this.decayed=0;this.clampAdj=0;this.discarded=0;
 ...
 deposit(x:number,y:number,amount:number,interval:Interval|null):number{
  ...
  // never silently dropped: lineage wasteProduced means deposited mass, and
  // the accounting identity reports saturated loss separately.
  let lost=amount-add;
  if(lost>0)this.discarded+=lost;
  if(add<=0)return 0;
